...
### 1) الاحتواء — 🟡 مُنفَّذ جزئيًّا

- **المفهوم**: شجرة «متبوع → تابع» (شركة تعليمية تحتوي مجمعًا/جامعة…، المجمع يحتوي
  مدرسة/روضة، الجامعة تحتوي كلية).
- **التقني (مُنفَّذ)**:
  - عمود `tenants.parent_tenant_id` (self-FK، nullable) — قائمة جوار. علاقات
    `Tenant::parent()/children()/scopeTopLevel()/descendantIds()`.
  - قواعد النوع في `App\Enums\EntityType` (`allowedChildTypes`/`isContainer`/
...
- عند **تنفيذ** بندٍ مُعلَّم 🔴 (كالمفوَّض)، حدِّث **كلتا** الطبقتين:
