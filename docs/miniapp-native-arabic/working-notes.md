...
5. **هل كتابات الكاش ذرّية أمام قتلٍ في منتصف الكتابة؟**
   `writeAsBytes(flush: true)` ثم تقليم الأشقّاء؛ والقتل **بينهما** يترك ملفين
   يتحمّلهما المفتاح — لكن القتل **أثناء** الكتابة يترك `.evc` مبتوراً ستعيده
   `read()` بكل رضا. `[افتراض]` (`edges.md` §5)
