...
ولا توجد طريقة دفاعية للنظر إلى مثل هذه القيمة — **و`containsKey` ليست منها.**
كُتب ذلك الحارس وشُحن ثم **رُدَّ على الجهاز**: تعليبه لا يطابق تعليب
`operator[]` (وهو معامل يعالجه dart_eval معالجة خاصة)، فصارت كل قراءة محروسة
تسقط إلى بديلها. اقرأ التعليق داخل `OktaJson.at`
(`okta-app/lib/features/miniapps/runtime/shared/okta_kit.dart:67`) قبل إعادته.
`[مؤكَّد — الردّ موجود في المصدر المشحون]`

فالدفاع عند المنبع، في الحمولة: أصدِر كل حقل يقرؤه التطبيق المصغّر في كل ردّ،
بقيمة `null` حين لا قيمة. مفتاح **موجود** قيمته null آمن، والغياب وحده قاتل.

و`OktaJson` يغطّي **الشكل** الخاطئ لا **الغياب**: `at` تستبعد مصدراً ليس كائناً،
لكن `strOr`/`number`/`list`/`rows` تُجري بعدها `is` على ما أعادته — وتلك هي
الرمية. و`OktaJson.flag` هي القارئة الوحيدة الآمنة أمام الغياب (`== true` لا
`is`). `[مؤكَّد]`

> **انحراف مفتوح**: تعليق `strOr` ما زال يدّعي أن `at` «ترفض فهرسة مفتاح غائب»،
> وذلك وصفٌ للنسخة المردودة. `[مؤكَّد — التعليق والكود متعارضان في الملف
> المشحون]`
