...
| المداخل الإضافية لكل نوع | `mobile_versions[]` على صفّ الجمهور — كل صفّ مقيَّد بمحور واحد: `min_contract` **أو** `min_app_version` | `mobile.audiences[].versions[]` ← `versions` لكل جمهور (`NormalizeMobileAudiences`): صفوف العقد أولاً بالأعلى أرضيةً، ثم صفوف الإصدار بالأحدث حدّاً، ولكلٍّ رمز حدّه (`c28` / `1.2.0`) | — (الانتقاء في الخادم؛ لا يسافر إلا المدخل والعقد المُختاران، ومعهما `e=<الحدّ>` في الرابط الموقَّع) | — (لا شيء جديد؛ الهاتف يبلّغ `X-App-Version`/`X-App-Contract` ويستلم مدخلاً واحداً) |
