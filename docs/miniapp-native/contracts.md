...
| Extra per-type entries | `mobile_versions[]` on the audience row — each line bound on ONE axis: `min_contract` **or** `min_app_version` | `mobile.audiences[].versions[]` → `versions` per audience (`NormalizeMobileAudiences`): contract-bound lines first by highest floor, then version-bound by newest bound, each carrying its `bound` token (`c28` / `1.2.0`) | — (the pick is server-side; only the chosen `entry`/`min_contract` travel, plus `e=<bound>` in the signed URL) | — (nothing new; the phone reports `X-App-Version`/`X-App-Contract` and receives one entry) |
